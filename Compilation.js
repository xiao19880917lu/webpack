...
class Compilation extends Tapable {
	...
	buildModule(module, optional, origin, dependencies, thisCallback) {
		...
		/** luyongfang 在buildModule生命周期内，调用用户注册过的钩子 */
		/**
		 * 1. webpack.config.js中配置plugins数组
		 * 2. 在webpack/lib/webpack.js中 判断options.plugins, 然后执行哥哥plugin的apply方法
		 * 3. 如plugin中 注册了compiler.hooks.compile.tap('testPlugin', fn), compiler是new Compiler()实例, 
		 *    compiler.hooks是实例的全局对象, compiler.hooks.compile指向了Tapable的hooks。 然后调用tapable的tap事件注册[{type: 'sync', name: 'TestPlugin', fn: fn}]
		 * 4. 在如buildModule等特定生命周期进行调用，注册的plugin Hooks
		 * 5. 如在compiler阶段 this.hooks.compile.call(params); this.hooks.make.callAsync(), 
		 * 6. 如在compilation阶段 this.hooks.optimizeChunks compilation.hooks.**钩子等调用
		*/
		...
		asyncLib.forEach(
			...
			(item, callback) => {
				...
				semaphore.acquire(() => {
					 ...
					 * 3. this.hooks.resolver: 而this.hooks.resolver的注册是在NormalModuleFactory实例化时。
					 * 4. afterResolver
					 * 5. this.hooks.resolver.tap 注册时会解析loaderResolver, normalResolver
					*/
					factory.create(
						{
							contextInfo: {
